...
void Drive::parseDrive(string s){
//	cout << "Drive::parseDrive"<< s << endl;
	string str = s.substr(s.find("\"plays\":"));
	string search="";

	int loc = 0,next = 0, start =0, end =0, exit = 0;
	Play *p;
	...
	while(exit == 0){
		...
		if(loc != -1){
			p = new Play();
			next = s.find("\"sp\":", loc+4);

			if(next !=-1){
				p->parsePlay(s.substr(loc,next-loc));
			}else{
				p->parsePlay(s.substr(loc+4));
				  exit =1;
			}
			plays.push_back(p);
		//cout << "FOUND: " << make_string1(i) << ": " << loc << endl;
		}
	}
}
